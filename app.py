...
if kakao_key:
    st.sidebar.success("카카오맵 API 활성화 (보완 데이터 ↑)")
else:
    st.sidebar.info("카카오맵 API 비활성화 (선택)")
    with st.sidebar.expander("카카오맵 추가하기 (무료)"):
        st.markdown(
            "**효과**: 네이버 지도와 다른 데이터셋. 네이버 지도가 못 찾는 회사 보강.\n\n"
            "**비용**: 무료 (일 30만 호출)\n\n"
            "**설정**:\n"
            "1. https://developers.kakao.com 가입 → 내 애플리케이션 → 추가하기\n"
            "2. 앱 키 → **REST API 키** 복사\n"
            "3. Streamlit Cloud Secrets에 추가:\n"
            "   ```\n"
...
if st.button("🔎 검색 시작", type="primary"):
    ...
    ccols[1].metric("AI확인", conf["AI확인"], help="Claude AI 단독")
    ccols[2].metric("지도확인", conf["지도확인"], help="네이버 지도 단독")
    ccols[3].metric("카카오맵확인", conf["카카오맵확인"], help="카카오맵 단독")
    ccols2 = st.columns(4)
    ccols2[0].metric("홈페이지확인", conf["홈페이지확인"], help="공식 홈페이지 단독")
    ccols2[1].metric("검색결과확인", conf["검색결과확인"], help="네이버 웹 검색 스니펫")
    ccols2[2].metric("잡포털확인", conf["잡포털확인"], help="잡코리아·사람인 단독")
    ccols2[3].metric("찾지못함", conf["찾지못함"])
